...
const AverageTickectCount = ({ selectedLocation }) => {
   ...
   const weeklyPercentage = ((weeklyAvgTicketActual - weeklyAvgTicketTarget) / weeklyAvgTicketTarget) * 100;
   
   // Previous week (using 95% as example for avg ticket)
   const prevWeekAvgTicket = weeklyAvgTicketActual * 0.95;
   const weekOnWeekPercentage = ((weeklyAvgTicketActual - prevWeekAvgTicket) / prevWeekAvgTicket) * 100;
  return (
    <div className="p-4 md:p-6 space-y-6 bg-white shadow-lg rounded-xl mx-4 md:mx-6">
      <div className="flex flex-col md:flex-row w-full md:justify-between md:items-center gap-4">
        ...
        <h2 className="text-base md:text-lg font-semibold text-gray-700 mb-2">Average Ticket Value for {selected}</h2>
      <p className="text-gray-500 text-xs md:text-sm mb-4">
        This shows daily and weekly actual vs target with week-on-week average ticket value comparison.
      </p>
      </div>
        <div className="relative inline-block text-left self-start md:self-auto">
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <AvgTicketCard
                    title="Daily (Today)"
                    actual={dailyAvgTicketActual}
                    target={dailyAvgTicketTarget}
                    percentage={dailyPercentage}
                    bgColor="bg-red-100 border-red-200"
                  />
                  <AvgTicketCard
                    title="Weekly (Current)"
                    actual={weeklyAvgTicketActual}
                    target={weeklyAvgTicketTarget}
                  ...
                  <AvgTicketCard
                    title="Week-On-Week"
                    actual={weeklyAvgTicketActual}
                    target={Math.round(prevWeekAvgTicket)}
      ...
      export default AverageTickectCount;
